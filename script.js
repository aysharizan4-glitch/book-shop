...
adminBtn.addEventListener('click', () => {
  ...
  if (password === 'aliya_sister0109') { // Change your password here
...
function renderProducts() {
  ...
  products.forEach((product, index) => {
    ...
    adminCard.innerHTML = `
      <h4>${product.name}</h4>
      <img src="${product.image_url}" alt="${product.name}" style="width:80px;height:100px;object-fit:cover;margin-bottom:10px;">
      <p>Price: LKR ${parseFloat(product.price).toFixed(2)}</p>
      <p>Category: ${product.category}</p>
      <p>${product.description}</p>
      <p>Stock: <span style="padding:2px 6px;border-radius:5px;color:#fff;font-size:12px; background:${product.in_stock ? '#28a745' : '#dc3545'}">${product.in_stock ? 'In Stock' : 'Out of Stock'}</span></p>
  ...
  products.forEach((product, index) => {
    ...
    productCard.innerHTML = `
      ...
      <img src="${product.image_url}" alt="${product.name}" style="width:150px;height:150px;object-fit:cover;margin-bottom:10px;">
...
productFormEl.addEventListener('submit', async (e) => {
  ...
  let image_url = "https://via.placeholder.com/150?text=No+Image";
  ...
  const { data, error } = await supabase
    .from('products')
    .insert([{ name, price, category, description, in_stock: inStock, featured, image_url }]);
...
// ================= Delete / Edit Product =================
adminProductListEl.addEventListener('click', async (e) => {
...
productListEl.addEventListener('click', (e) => {
  ...
  if (e.target.classList.contains('order-btn')) {
    const product = products[index];
    const message = `Hello AYSHLYNE,%0A%0AI want to order:%0A- ${product.name}%0A- Price: LKR ${parseFloat(product.price).toFixed(2)}%0A- Category: ${product.category}%0A- ${product.in_stock ? 'In Stock' : 'Out of Stock'}%0A%0AThanks!`;
    const whatsappURL = `https://wa.me/94788878600?text=${message}`;
    window.open(whatsappURL, '_blank');
  }
});
